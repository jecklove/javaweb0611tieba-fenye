...
public class PostDao {
	 ...
	 * @throws SQLException 
	 */
	public List<Post> findList(Long startIndex, Long pageSize) throws SQLException {
		QueryRunner qr = new QueryRunner(DataSourceUtil.getDataSource());
		String sql = "SELECT post.*,`user`.nickname,COUNT(post_id) AS comment_count FROM post INNER JOIN `user` ON post.user_id = `user`.id LEFT JOIN `comment` ON post.id = `comment`.post_id GROUP BY post.id ORDER BY post.create_time DESC LIMIT ?,?";
		List<Post> result = qr.query(sql, new BeanListHandler<>(Post.class), startIndex, pageSize);
		return result;
	}
	...
	public Long count() throws SQLException {
		...
		Long result = (Long)qr.query(sql, new ScalarHandler<>(1));
		return result;
	}
